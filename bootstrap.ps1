...
$EnvVars = [ordered]@{
    ...
    # psmux ignores tmux's XDG search entirely; this is the only hook it gives
    # us. Points at the generated entry file, not tmux.conf directly.
    'PSMUX_CONFIG_FILE'    = "$Repo\.config\tmux\psmux.conf"
...
$psmuxEntry = "$Repo\.config\tmux\psmux.conf"
$fwd        = { param($p) $p -replace '\\', '/' }

$entryBody = @"
# Generated by bootstrap.ps1. Do not edit - gitignored.
# psmux entry point: absolute paths only, see bootstrap.ps1 for why.
source-file "$(& $fwd "$Repo\.config\tmux\tmux.conf")"
source-file "$(& $fwd "$Repo\.config\tmux\plugins.windows.conf")"
"@
...
if ($null -ne $curEntry -and $curEntry.Trim() -eq $entryBody.Trim()) {
    Write-Skip 'psmux.conf'
} elseif ($DryRun) {
    Write-Plan 'would generate .config/tmux/psmux.conf'
} else {
    Set-Content -LiteralPath $psmuxEntry -Value $entryBody -Encoding ascii
    Write-Do 'generated .config/tmux/psmux.conf'
}
