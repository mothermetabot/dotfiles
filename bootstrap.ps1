...
# $HOME points at the workspace root (~\home), not at %USERPROFILE%. Unix-ish
# tools resolve ~ through $HOME, so without this they look in
# ~\home\.config\<tool> and find nothing - which is exactly how psmux silently
# ran with stock defaults instead of .config/tmux/tmux.conf.
#
# Aliasing the whole directory rather than adding a per-tool fallback means any
# future XDG tool works under either home with no extra wiring.
if ($env:HOME -and $env:HOME.TrimEnd('\') -ne $Target.TrimEnd('\')) {
    $Junctions += @{
        Name   = 'HOME/.config alias'
        Link   = "$($env:HOME.TrimEnd('\'))\.config"
        Target = $Xdg
    }
}
...
Write-Head '[5b] psmux entry config'

# psmux does NOT follow tmux's XDG search. Its own order is:
#   %USERPROFILE%\.psmux.conf, .psmuxrc, .tmux.conf, .config\psmux\psmux.conf
# so .config/tmux/tmux.conf would never be found. It does honour
# $PSMUX_CONFIG_FILE, which is what we use (see $EnvVars).
#
# It also expands neither `~` nor `$HOME` in source-file, and resolves relative
# paths against the CWD rather than the config file's directory - all three
# verified against psmux 3.3.8. So the entry file below uses absolute paths.
# Without it, the `source-file ~/.config/tmux/plugins.conf` line in tmux.conf
# fails silently and the PPM plugin tail never loads.
# Written to %USERPROFILE%\.psmux.conf - the FIRST path in psmux's own search
# order - so no environment variable is needed and nothing generated lives in
# the repo. Same pattern as the $PROFILE and ~/.gitconfig stubs.
$psmuxEntry = "$Target\.psmux.conf"
$fwd        = { param($p) $p -replace '\\', '/' }

$tmuxDir = & $fwd "$Repo\.config\tmux"

$entryBody = @"
# Generated by bootstrap.ps1. Do not edit - edit the repo files instead.
#
# This is an ENTRY STUB, not a config. The actual configuration is the shared
# .config/tmux/tmux.conf, which is byte-identical on Windows and Linux. This
# file exists only because psmux cannot reach it on its own: it does not follow
# tmux's XDG search, and expands neither ~ nor `$HOME in source-file.
#
# Linux needs the same stub for the opposite reason - the shared file cannot
# carry a platform-specific plugin tail - and gets one at
# ~/.config/tmux/tmux.conf from bootstrap.sh.
source-file "$tmuxDir/tmux.conf"
source-file "$tmuxDir/plugins.windows.conf"

# Bindings that need an absolute script path. They live here rather than in
# tmux.conf because psmux expands neither ~ nor `$HOME.
bind-key b display-popup -E -w 80% -h 70% "powershell -NoProfile -ExecutionPolicy Bypass -File \`"$tmuxDir/scripts/fzf-window.ps1\`""
bind-key s display-popup -E -w 60% -h 50% "powershell -NoProfile -ExecutionPolicy Bypass -File \`"$tmuxDir/scripts/fzf-session.ps1\`""
bind-key r source-file "$(& $fwd $psmuxEntry)" \; display-message "psmux config reloaded"
"@

$curEntry = if (Test-Path -LiteralPath $psmuxEntry) { Get-Content -LiteralPath $psmuxEntry -Raw } else { $null }
if ($null -ne $curEntry -and $curEntry.Trim() -eq $entryBody.Trim()) {
    Write-Skip $psmuxEntry
} elseif ($DryRun) {
    Write-Plan "would generate $psmuxEntry"
} else {
    Set-Content -LiteralPath $psmuxEntry -Value $entryBody -Encoding ascii
    Write-Do "generated $psmuxEntry"
}
