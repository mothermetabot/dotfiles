...
head '[3/6] tmux plugin tail'

# tmux.conf is shared with Windows; only the plugin manager differs.
if [ "$DRY_RUN" = 1 ]; then
  plan 'would set .config/tmux/plugins.conf from plugins.linux.conf'
else
  cp "$REPO/.config/tmux/plugins.linux.conf" "$REPO/.config/tmux/plugins.conf"
  did 'plugins.conf -> plugins.linux.conf'
fi
