...
head '[3/6] tmux entry config'

# .config/tmux is held back from stow (see .stow-local-ignore) and the entry
# point is generated here instead, mirroring what bootstrap.ps1 does for psmux.
# The shared tmux.conf carries no source-file line for the plugin tail, because
# psmux expands neither ~ nor $HOME and no single line works on both platforms.
if [ "$DRY_RUN" = 1 ]; then
  plan "would generate ~/.config/tmux/tmux.conf sourcing the repo + plugins.linux.conf"
else
  mkdir -p "$HOME/.config/tmux"
  chmod +x "$REPO/.config/tmux/scripts/"*.sh 2>/dev/null || true
  cat > "$HOME/.config/tmux/tmux.conf" <<EOF
# Generated by bootstrap.sh. Do not edit - edit the repo files instead.
source-file "$REPO/.config/tmux/tmux.conf"
source-file "$REPO/.config/tmux/plugins.linux.conf"

# Bindings that need an absolute script path. Kept here rather than in
# tmux.conf so the same shared file works under psmux, which expands neither
# ~ nor \$HOME.
bind-key b display-popup -E -w 80% -h 70% "$REPO/.config/tmux/scripts/fzf-window.sh"
bind-key s display-popup -E -w 60% -h 50% "$REPO/.config/tmux/scripts/fzf-session.sh"
bind-key r source-file "\$HOME/.config/tmux/tmux.conf" \\; display-message "tmux config reloaded"
EOF
  did 'generated ~/.config/tmux/tmux.conf'
fi
